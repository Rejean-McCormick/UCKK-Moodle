...
/**
 * Return revision history for an archive item.
 *
 ...
 * Return revision history for an archive item.
 */
class get_revisions extends external_api {
    ...
    public static function execute(int $cmid, int $itemid): array {
        ...
        $revisions = [];
        foreach (self::load_revisions((int)$item->id) as $revision) {
            $revisions[] = [
                'id' => (int)$revision->id,
                'versionno' => (int)($revision->versionno ?? 1),
                'changereason' => (string)($revision->changereason ?? ''),
                'status' => (string)($revision->status ?? ''),
                'visibility' => (string)($revision->visibility ?? ''),
                'previousstate' => (string)($revision->previousstate ?? ''),
                'newstate' => (string)($revision->newstate ?? ''),
                'timecreated' => (int)($revision->timecreated ?? 0),
            ];
        }

        return ['revisions' => $revisions, 'warnings' => []];
    }
        ...
        return new external_single_structure([
            'revisions' => new external_multiple_structure(
                new external_single_structure([
                    ...
                    'changereason' => new external_value(PARAM_RAW, 'Change reason'),
                    'status' => new external_value(PARAM_ALPHANUMEXT, 'Status'),
                    'visibility' => new external_value(PARAM_ALPHANUMEXT, 'Visibility'),
                    'previousstate' => new external_value(PARAM_RAW, 'Previous state JSON'),
                    'newstate' => new external_value(PARAM_RAW, 'New state JSON'),
