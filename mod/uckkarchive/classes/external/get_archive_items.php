...
/**
 * Return permission-filtered archive items.
 *
 ...
 * Return permission-filtered archive items.
 */
class get_archive_items extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'itemtype' => new external_value(PARAM_ALPHANUMEXT, 'Item type filter', VALUE_DEFAULT, ''),
            'validationstate' => new external_value(PARAM_ALPHANUMEXT, 'Validation state filter', VALUE_DEFAULT, ''),
            'limit' => new external_value(PARAM_INT, 'Limit', VALUE_DEFAULT, 0),
            'offset' => new external_value(PARAM_INT, 'Offset', VALUE_DEFAULT, 0),
        ]);
    }

    public static function execute(int $cmid, string $status = '', string $itemtype = '', string $validationstate = '', int $limit = 0, int $offset = 0): array {
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'status', 'itemtype', 'validationstate', 'limit', 'offset'));
        [$course, $cm, $archive, $context] = self::load_page($params['cmid']);
        self::validate_context($context);
        require_capability('mod/uckkarchive:view', $context);

        $filters = [];
        foreach (['status', 'itemtype', 'validationstate'] as $key) {
            if ($params[$key] !== '') {
                $filters[$key] = $params[$key];
            }
        }

        $records = \uckkarchive_get_visible_items($archive, $context, null, $filters, $params['limit'], $params['offset']);
        $items = [];
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            ...
            'count' => new external_value(PARAM_INT, 'Returned item count'),
