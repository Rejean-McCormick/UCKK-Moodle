...
/**
 * Revise an existing archive item.
 *
 ...
 * Revise an existing archive item.
 */
class revise_item extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'summary' => new external_value(PARAM_RAW, 'New summary', VALUE_DEFAULT, ''),
            'content' => new external_value(PARAM_RAW, 'New content', VALUE_DEFAULT, ''),
            'publicsummary' => new external_value(PARAM_RAW, 'New public summary', VALUE_DEFAULT, ''),
            'status' => new external_value(PARAM_ALPHANUMEXT, 'New status', VALUE_DEFAULT, ''),
            'visibility' => new external_value(PARAM_ALPHANUMEXT, 'New visibility', VALUE_DEFAULT, ''),
            'reason' => new external_value(PARAM_TEXT, 'Revision reason'),
        ]);
    }

    public static function execute(int $cmid, int $itemid, string $title = '', string $summary = '', string $content = '', string $publicsummary = '', string $status = '', string $visibility = '', string $reason = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'itemid', 'title', 'summary', 'content', 'publicsummary', 'status', 'visibility', 'reason'));
        [$course, $cm, $archive, $context] = self::load_page($params['cmid']);
        self::validate_context($context);
        require_capability('mod/uckkarchive:reviseitem', $context);

        $olditem = \uckkarchive_get_item($params['itemid'], (int)$archive->id);
        if (!\uckkarchive_can_revise_item($archive, $olditem, $cm, $context, $USER)) {
            throw new \required_capability_exception($context, 'mod/uckkarchive:reviseitem', 'nopermissions', '');
        }

        $update = (object)[
            'id' => (int)$olditem->id,
            'title' => $params['title'] !== '' ? trim($params['title']) : (string)$olditem->title,
            'summary' => $params['summary'] !== '' ? $params['summary'] : (string)($olditem->summary ?? ''),
            'content' => $params['content'] !== '' ? $params['content'] : (string)($olditem->content ?? ''),
            'contentformat' => FORMAT_HTML,
            'publicsummary' => $params['publicsummary'] !== '' ? $params['publicsummary'] : (string)($olditem->publicsummary ?? ''),
            'status' => $params['status'] !== '' ? \uckkarchive_normalise_status($params['status']) : (string)$olditem->status,
            'visibility' => $params['visibility'] !== '' ? \uckkarchive_normalise_visibility($params['visibility']) : (string)$olditem->visibility,
            'versionno' => max(1, (int)($olditem->versionno ?? 1)) + 1,
            'modifiedby' => (int)$USER->id,
            'timemodified' => time(),
        ];
        $DB->update_record('uckkarchive_item', $update);
        $newitem = $DB->get_record('uckkarchive_item', ['id' => $olditem->id], '*', MUST_EXIST);

        $revisionid = 0;
        if ($DB->get_manager()->table_exists('uckkarchive_rev')) {
            $revisionid = \uckkarchive_create_revision($olditem, $newitem, $params['reason'], (int)$USER->id, $newitem->integritycaseid ?? null);
        }

        if (class_exists(archive_item_revised::class)) {
            $event = archive_item_revised::create([
                'objectid' => (int)$newitem->id,
                'context' => $context,
                'other' => [
                    ...
                    'visibility' => (string)$newitem->visibility,
                    'versionno' => (int)$newitem->versionno,
                ],
            ]);
            $event->add_record_snapshot('course', $course);
            $event->add_record_snapshot('course_modules', $cm);
            $event->add_record_snapshot('uckkarchive', $archive);
            $event->add_record_snapshot('uckkarchive_item', $newitem);
            $event->trigger();
        }

        return [
            'item' => self::export_item($newitem),
            'revisionid' => $revisionid,
            'warnings' => [],
        ];
    }
