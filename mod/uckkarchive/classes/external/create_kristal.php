...
/**
 * Create a Kristal from an archive item.
 *
 ...
 * Create a Kristal from an archive item.
 */
class create_kristal extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'itemid' => new external_value(PARAM_INT, 'Source item id'),
            'title' => new external_value(PARAM_TEXT, 'Kristal title'),
            'claim' => new external_value(PARAM_RAW, 'Claim'),
            'body' => new external_value(PARAM_RAW, 'Body', VALUE_DEFAULT, ''),
            'evidence' => new external_value(PARAM_RAW, 'Evidence summary', VALUE_DEFAULT, ''),
            'confidence' => new external_value(PARAM_INT, 'Confidence from 0 to 100', VALUE_DEFAULT, 0),
    ...
    public static function execute(int $cmid, int $itemid, string $title, string $claim, string $body = '', string $evidence = '', int $confidence = 0, string $visibility = 'course', string $provenance = 'human', string $metadatajson = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'itemid', 'title', 'claim', 'body', 'evidence', 'confidence', 'visibility', 'provenance', 'metadatajson'));
        [$course, $cm, $archive, $context] = self::load_page($params['cmid']);
        self::validate_context($context);
        require_capability('mod/uckkarchive:validateitem', $context);

        if (!$DB->get_manager()->table_exists('uckkarchive_kristal')) {
            throw new \moodle_exception('missingtable', 'error', '', 'uckkarchive_kristal');
        }
        ...
        $record = (object)[
            ...
            'confidence' => max(0, min(100, (int)$params['confidence'])),
        ...
        $kristalid = (int)$DB->insert_record('uckkarchive_kristal', $record);

        return ['kristalid' => $kristalid, 'warnings' => []];
    }
