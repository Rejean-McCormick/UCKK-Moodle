 ...
 * Update an existing Kristal.
 */
class update_kristal extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'kristalid' => new external_value(PARAM_INT, 'Kristal id'),
            'title' => new external_value(PARAM_TEXT, 'Title', VALUE_DEFAULT, ''),
            'claim' => new external_value(PARAM_RAW, 'Claim', VALUE_DEFAULT, ''),
            'body' => new external_value(PARAM_RAW, 'Body', VALUE_DEFAULT, ''),
            'evidence' => new external_value(PARAM_RAW, 'Evidence', VALUE_DEFAULT, ''),
            'confidence' => new external_value(PARAM_INT, 'Confidence', VALUE_DEFAULT, -1),
            ...
            'validationstate' => new external_value(PARAM_ALPHANUMEXT, 'Validation state', VALUE_DEFAULT, ''),
        ]);
    }

    public static function execute(int $cmid, int $kristalid, string $title = '', string $claim = '', string $body = '', string $evidence = '', int $confidence = -1, string $status = '', string $visibility = '', string $validationstate = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'kristalid', 'title', 'claim', 'body', 'evidence', 'confidence', 'status', 'visibility', 'validationstate'));
        [$course, $cm, $archive, $context] = self::load_page($params['cmid']);
        self::validate_context($context);
        require_capability('mod/uckkarchive:reviseitem', $context);

        $kristal = $DB->get_record('uckkarchive_kristal', ['id' => $params['kristalid'], 'archiveid' => $archive->id], '*', MUST_EXIST);
        $update = (object)[
            'id' => (int)$kristal->id,
            'title' => $params['title'] !== '' ? trim($params['title']) : (string)$kristal->title,
            'claim' => $params['claim'] !== '' ? $params['claim'] : (string)($kristal->claim ?? ''),
            'body' => $params['body'] !== '' ? $params['body'] : (string)($kristal->body ?? ''),
            'bodyformat' => FORMAT_HTML,
            'evidence' => $params['evidence'] !== '' ? $params['evidence'] : (string)($kristal->evidence ?? ''),
            'confidence' => $params['confidence'] >= 0 ? max(0, min(100, $params['confidence'])) : (int)($kristal->confidence ?? 0),
            'status' => $params['status'] !== '' ? \uckkarchive_normalise_status($params['status']) : (string)$kristal->status,
            'visibility' => $params['visibility'] !== '' ? \uckkarchive_normalise_visibility($params['visibility']) : (string)$kristal->visibility,
            'validationstate' => $params['validationstate'] !== '' ? \uckkarchive_normalise_validation_state($params['validationstate']) : (string)$kristal->validationstate,
            'versionno' => max(1, (int)($kristal->versionno ?? 1)) + 1,
            'modifiedby' => (int)$USER->id,
            'timemodified' => time(),
        ];
        $DB->update_record('uckkarchive_kristal', $update);

        return ['kristalid' => (int)$kristal->id, 'warnings' => []];
    }
