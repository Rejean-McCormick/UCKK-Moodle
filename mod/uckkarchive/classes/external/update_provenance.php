...
/**
 * Update provenance metadata for an item or proof.
 *
 ...
 * Update provenance metadata for an item or proof.
 */
class update_provenance extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'proofid' => new external_value(PARAM_INT, 'Proof id', VALUE_DEFAULT, 0),
            'source' => new external_value(PARAM_RAW, 'Source statement', VALUE_DEFAULT, ''),
            'origincomponent' => new external_value(PARAM_COMPONENT, 'Origin component', VALUE_DEFAULT, ''),
            'originid' => new external_value(PARAM_INT, 'Origin id', VALUE_DEFAULT, 0),
            'evidence' => new external_value(PARAM_RAW, 'Evidence summary', VALUE_DEFAULT, ''),
            'sourcedate' => new external_value(PARAM_INT, 'Source date', VALUE_DEFAULT, 0),
    ...
    public static function execute(int $cmid, int $itemid, int $proofid = 0, string $source = '', string $origincomponent = '', int $originid = 0, string $evidence = '', int $sourcedate = 0, string $visibility = 'course', string $provenance = 'human', string $metadatajson = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'itemid', 'proofid', 'source', 'origincomponent', 'originid', 'evidence', 'sourcedate', 'visibility', 'provenance', 'metadatajson'));
        ...
        if ($DB->get_manager()->table_exists('uckkarchive_prov')) {
            $existing = null;
            if ($params['proofid'] > 0) {
                $existing = $DB->get_record('uckkarchive_prov', ['itemid' => $item->id, 'proofid' => $params['proofid']], '*');
            }
            if (!$existing) {
                $existing = $DB->get_record('uckkarchive_prov', ['itemid' => $item->id, 'proofid' => null], '*');
            }
            ...
            if ($existing) {
                $update = (object)[
                    ...
                    'sourcedate' => $params['sourcedate'] > 0 ? $params['sourcedate'] : null,
                ...
                $DB->update_record('uckkarchive_prov', $update);
                $recordid = (int)$existing->id;
            } else {
                $recordid = \uckkarchive_create_provenance_record(
                    (int)$archive->id,
                    (int)$item->id,
                    (int)$context->id,
                    (int)$USER->id,
                    $params['origincomponent'],
                    (int)$params['originid'],
                    $params['source'],
                    $params['provenance'],
                    [
                ...
                if ($params['proofid'] > 0) {
                    $DB->set_field('uckkarchive_prov', 'proofid', $params['proofid'], ['id' => $recordid]);
                }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'recordid' => new external_value(PARAM_INT, 'Provenance record id'),
