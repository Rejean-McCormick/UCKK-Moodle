...
/**
 * Return provenance panel data for one archive item.
 *
 ...
 * Return provenance panel data for one archive item.
 */
class get_provenance_panel extends external_api {
    ...
    public static function execute(int $cmid, int $itemid): array {
        ...
        $panel = new provenance_panel((int)$item->id, (int)$context->id, self::load_provenance((int)$item->id), [
            'canviewrestricted' => has_capability('mod/uckkarchive:viewrestricted', $context),
        ]);
        $contextdata = $panel->export_for_template($OUTPUT);

        return [
            'templatecontext' => json_encode($contextdata, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
            'html' => $OUTPUT->render_from_template('mod_uckkarchive/provenance_panel', $contextdata),
            'warnings' => [],
        ];
    }
