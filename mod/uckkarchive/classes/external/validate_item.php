 ...
 * Validate an archive item.
 */
class validate_item extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'validationstate' => new external_value(PARAM_ALPHANUMEXT, 'Validation state', VALUE_DEFAULT, 'verified'),
            'status' => new external_value(PARAM_ALPHANUMEXT, 'Status', VALUE_DEFAULT, 'validated'),
            'visibility' => new external_value(PARAM_ALPHANUMEXT, 'Visibility', VALUE_DEFAULT, 'course'),
            'validationnotes' => new external_value(PARAM_RAW, 'Validation notes', VALUE_DEFAULT, ''),
            'publicsummary' => new external_value(PARAM_RAW, 'Public summary', VALUE_DEFAULT, ''),
            'restrictednotes' => new external_value(PARAM_RAW, 'Restricted notes', VALUE_DEFAULT, ''),
            'checksjson' => new external_value(PARAM_RAW, 'Validation checks JSON', VALUE_DEFAULT, ''),
        ]);
    }

    public static function execute(int $cmid, int $itemid, string $validationstate = 'verified', string $status = 'validated', string $visibility = 'course', string $validationnotes = '', string $publicsummary = '', string $restrictednotes = '', string $checksjson = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'itemid', 'validationstate', 'status', 'visibility', 'validationnotes', 'publicsummary', 'restrictednotes', 'checksjson'));
        ...
        $olditem = clone $item;
        $metadata = \uckkarchive_decode_metadata($item->metadata ?? null);
        $checks = $params['checksjson'] !== '' ? (json_decode($params['checksjson'], true) ?: []) : [];
        $metadata['validation'] = [
            'notes' => $params['validationnotes'],
            'restrictednotes' => $params['restrictednotes'],
            'checks' => $checks,
        ...
        $update = (object)[
            ...
            'publicsummary' => $params['publicsummary'] !== '' ? $params['publicsummary'] : (string)($item->publicsummary ?? ''),
            'versionno' => max(1, (int)($item->versionno ?? 1)),
            'modifiedby' => (int)$USER->id,
            'timemodified' => time(),
            'metadata' => \uckkarchive_encode_metadata($metadata),
        ];
        $DB->update_record('uckkarchive_item', $update);
        $item = $DB->get_record('uckkarchive_item', ['id' => $item->id], '*', MUST_EXIST);

        if ($DB->get_manager()->table_exists('uckkarchive_rev')) {
            \uckkarchive_create_revision($olditem, $item, $params['validationnotes'] !== '' ? $params['validationnotes'] : get_string('itemvalidated', 'uckkarchive'), (int)$USER->id, $item->integritycaseid ?? null);
        }

        if (class_exists(archive_item_validated::class)) {
