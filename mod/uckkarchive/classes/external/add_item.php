...
class add_item extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'itemtype' => new external_value(PARAM_ALPHANUMEXT, 'Item type', VALUE_DEFAULT, 'proof'),
            'summary' => new external_value(PARAM_RAW, 'Internal summary', VALUE_DEFAULT, ''),
            'content' => new external_value(PARAM_RAW, 'Main content', VALUE_DEFAULT, ''),
            'publicsummary' => new external_value(PARAM_RAW, 'Public summary', VALUE_DEFAULT, ''),
            'status' => new external_value(PARAM_ALPHANUMEXT, 'Initial status', VALUE_DEFAULT, 'draft'),
            'visibility' => new external_value(PARAM_ALPHANUMEXT, 'Visibility', VALUE_DEFAULT, 'course'),
            'validationstate' => new external_value(PARAM_ALPHANUMEXT, 'Validation state', VALUE_DEFAULT, 'unverified'),
            'provenance' => new external_value(PARAM_ALPHANUMEXT, 'Provenance', VALUE_DEFAULT, 'human'),
            'origincomponent' => new external_value(PARAM_COMPONENT, 'Origin component', VALUE_DEFAULT, ''),
            'originid' => new external_value(PARAM_INT, 'Origin object id', VALUE_DEFAULT, 0),
            'metadatajson' => new external_value(PARAM_RAW, 'Metadata JSON', VALUE_DEFAULT, ''),
            'reason' => new external_value(PARAM_TEXT, 'Creation reason', VALUE_DEFAULT, ''),
        ]);
    }

    public static function execute(int $cmid, string $title, string $itemtype = 'proof', string $summary = '', string $content = '', string $publicsummary = '', string $status = 'draft', string $visibility = 'course', string $validationstate = 'unverified', string $provenance = 'human', string $origincomponent = '', int $originid = 0, string $metadatajson = '', string $reason = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'title', 'itemtype', 'summary', 'content', 'publicsummary', 'status', 'visibility', 'validationstate', 'provenance', 'origincomponent', 'originid', 'metadatajson', 'reason'));
        ...
        $record = (object)[
            ...
            'userid' => (int)$USER->id,
            'itemtype' => \uckkarchive_normalise_item_type($params['itemtype']),
            'title' => trim($params['title']),
            'summary' => $params['summary'],
            'content' => $params['content'],
            'contentformat' => FORMAT_HTML,
            'publicsummary' => $params['publicsummary'],
            'status' => \uckkarchive_normalise_status($params['status']),
            'validationstate' => \uckkarchive_normalise_validation_state($params['validationstate']),
            ...
            'origincomponent' => $params['origincomponent'] !== '' ? clean_param($params['origincomponent'], PARAM_COMPONENT) : null,
            'originobjectid' => $params['originid'] > 0 ? $params['originid'] : null,
            'provenancehash' => \uckkarchive_compute_provenance_hash([
                'title' => $params['title'],
                'content' => $params['content'],
                'publicsummary' => $params['publicsummary'],
                'origincomponent' => $params['origincomponent'],
        ...
        if ($DB->get_manager()->table_exists('uckkarchive_rev')) {
            \uckkarchive_create_revision((object)[], $item, $params['reason'] !== '' ? $params['reason'] : get_string('archiveitemcreated', 'uckkarchive'), (int)$USER->id);
        }
        ...
        if (class_exists(archive_item_created::class)) {
            $event = archive_item_created::create([
                ...
                'other' => [
                    ...
                    'validationstate' => (string)$item->validationstate,
                    'origincomponent' => (string)($item->origincomponent ?? ''),
                    'originid' => (int)($item->originobjectid ?? 0),
                ],
            ]);
        ...
        return ['item' => self::export_item($item), 'warnings' => []];
    }
