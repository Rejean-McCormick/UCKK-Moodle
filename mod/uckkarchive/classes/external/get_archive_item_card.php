...
/**
 * Return rendered item card data.
 *
 ...
 * Return rendered item card data.
 */
class get_archive_item_card extends external_api {
    ...
    public static function execute(int $cmid, int $itemid): array {
        ...
        $renderable = new archive_item_card(
            self::export_item($item),
            [
                'course' => $course,
                'cm' => $cm,
                'context' => $context,
                'proofs' => self::load_proofs((int)$item->id),
                'kristal' => self::load_kristal((int)$item->id) ?? [],
                'revisions' => self::load_revisions((int)$item->id),
                'exports' => self::load_exports((int)$item->id),
                'canviewrestricted' => has_capability('mod/uckkarchive:viewrestricted', $context),
            ]
        );
        $contextdata = $renderable->export_for_template($OUTPUT);

        return [
            'item' => self::export_item($item),
            'templatecontext' => json_encode($contextdata, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
            'html' => $OUTPUT->render_from_template('mod_uckkarchive/archive_item_card', $contextdata),
            'warnings' => [],
        ];
    }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            ...
            'templatecontext' => new external_value(PARAM_RAW, 'JSON template context'),
