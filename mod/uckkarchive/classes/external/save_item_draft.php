 ...
 * Save a draft archive item.
 */
class save_item_draft extends external_api {
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'itemid' => new external_value(PARAM_INT, 'Existing item id or 0', VALUE_DEFAULT, 0),
            ...
            'content' => new external_value(PARAM_RAW, 'Content HTML', VALUE_DEFAULT, ''),
    ...
    public static function execute(int $cmid, int $itemid = 0, string $title = '', string $itemtype = 'proof', string $summary = '', string $content = '', string $publicsummary = '', string $visibility = 'course', string $provenance = 'human', string $metadatajson = ''): array {
        global $DB, $USER;
        $params = self::validate_parameters(self::execute_parameters(), compact('cmid', 'itemid', 'title', 'itemtype', 'summary', 'content', 'publicsummary', 'visibility', 'provenance', 'metadatajson'));
        ...
        if ($params['metadatajson'] !== '') {
            $metadata = json_decode($params['metadatajson'], true) ?: [];
        }

        $now = time();
        if ($params['itemid'] > 0) {
            $item = \uckkarchive_get_item($params['itemid'], (int)$archive->id);
            if (!\uckkarchive_can_revise_item($archive, $item, $cm, $context, $USER)) {
            ...
            $item->id = (int)$DB->insert_record('uckkarchive_item', $item);
            $item = $DB->get_record('uckkarchive_item', ['id' => $item->id], '*', MUST_EXIST);
        }
