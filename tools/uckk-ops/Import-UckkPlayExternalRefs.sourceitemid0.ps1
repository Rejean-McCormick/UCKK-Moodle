...
# producing mojibake such as "b├®ton" or "ÔÇÖ".
...
function looks_mojibake($value) {
    ...
    return preg_match('/(├|┬|┼|ÔÇ|â€™|â€œ|â€|â€“|â€”|Ã©|Ã¨|Ãª|Ã«|Ã |Ã¢|Ã´|Ã»|Ã§)/u', $value) === 1;
}
...
function row_from_item($item, $sourcecatalog, $root, $youtubeCatalog, $target) {
    ...
    return [
        ...
        'publicationyear' => parse_year($item),
        'identifier' => $identifier,
        'identifiertype' => $identifiertype,
        'citation' => $title && $url ? $title . ' — ' . $url : $url,
        'metadata' => $metadata,
...
function apply_external_work($DB, &$result, $row, $target, $apply) {
    ...
    $record->publicationyear = $row['publicationyear'];
    ...
    $record->teachingnote = null;
    $record->culturalprotocolnote = null;
...
function apply_media($DB, &$result, $row, $target, $externalworkid, $apply) {
    ...
    $record->sourceid = $existing ? $existing->sourceid : null;
